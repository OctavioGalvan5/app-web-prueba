...
def obtener_indices_luz_fuerza(fecha_inicio: str, fecha_fin: str) -> list[Decimal]:
    """
    Devuelve la lista de índices Luz y Fuerza (uno por mes) para el rango:
    del PRIMER día del mes siguiente a `fecha_inicio` hasta el mes de `fecha_fin` (inclusive).
    Los índices se obtienen de la tabla `tabla_movilidades`, columna `RIPLYF_Luz_y_Fuerza`.
    """
    ...
    with engine.connect() as conn:
        stmt = text("""
            SELECT fechas, RIPLYF_Luz_y_Fuerza 
...
def grafico_linea_haber_luz_fuerza(fecha_inicio: str,
                                fecha_fin: str,
                                monto_inicial,
                                titulo: str = "Haber con aumentos Luz y Fuerza"):
    # 1) Serie calculada (valores por mes)
    serie = calcular_serie_luz_fuerza(fecha_inicio, fecha_fin, monto_inicial)
...
def grafico_comparativo_luz_fuerza_anses(fecha_inicio: str,
                                       fecha_fin: str,
                                       monto_inicial,
                                       serie_luz_fuerza: list,
                                       serie_anses: list,
                                       titulo: str = "Comparación: Movilidad Luz y Fuerza vs ANSES"):
    """
    ...
    n = min(len(serie_luz_fuerza), len(serie_anses), len(fechas))
    serie_luz_fuerza = serie_luz_fuerza[:n]
    ...
    y_luz_fuerza = [_to_two_dec(v) for v in serie_luz_fuerza]
    y_anses = [_to_two_dec(v) for v in serie_anses]

    hover_luz_fuerza = [_formato_pesos_ar(v) for v in serie_luz_fuerza]
    ...
    # Línea Luz y Fuerza (color morado)
    fig.add_trace(
        go.Scatter(
            x=fechas,
            y=y_luz_fuerza,
            mode="lines+markers",
            name="Movilidad Luz y Fuerza",
            text=[mes_es(d) for d in fechas],
            customdata=hover_luz_fuerza,
            hovertemplate="<b>%{text}</b><br>Luz y Fuerza: %{customdata}<extra></extra>",
...
def calcular_serie_luz_fuerza(fecha_inicio: str, fecha_fin: str,
                           monto_inicial) -> list:
    """
    Calcula la serie de montos multiplicando mes a mes por los índices Luz y Fuerza
    desde el PRIMER DÍA DEL MES SIGUIENTE a `fecha_inicio` hasta el mes de `fecha_fin` (inclusive).
    ...
    print(f"[LUZ Y FUERZA] Rango a calcular → desde: {start_dt} hasta: {end_dt}")

    if start_dt > end_dt:
        print("⚠️ [LUZ Y FUERZA] start_dt > end_dt: no hay meses a procesar en el rango.")
        return []
    ...
    if not rows:
        print("⚠️ [LUZ Y FUERZA] No se encontraron filas en 'tabla_movilidades' para el rango solicitado.")

    monto = Decimal(str(monto_inicial))
    lista_montos = []

    for fecha, indice in rows:
        print(f"[LUZ Y FUERZA] Mes: {fecha} | índice_luz_fuerza (RIPLYF)={indice}")
        try:
            idx_dec = Decimal(str(indice))
        except (InvalidOperation, TypeError):
            print("  → índice inválido/null, usando 1.0")
...
class CalculadorMovilidadLuzFuerza:
    ...
    def __init__(
            ...
            establecimiento,
            localidad,
            comparar_con_anses=False):  # opción de comparación
        # Datos del afiliado
        ...
        print(f"🔍 DEBUG CLASE LyF - comparar_con_anses: {self.comparar_con_anses}")

        # Resultados Luz y Fuerza
        self.serie_montos = calcular_serie_luz_fuerza(
        ...
        if self.comparar_con_anses:
            print("🔍 DEBUG CLASE LyF - Calculando serie ANSES...")
            self.serie_anses = calcular_serie_anses(
                fecha_inicio=self.periodo_desde,
                fecha_fin=self.periodo_hasta,
                monto_inicial=self.monto)
            self.monto_final_anses = self.serie_anses[-1] if self.serie_anses else None
            print(f"🔍 DEBUG CLASE LyF - Serie ANSES calculada: {len(self.serie_anses) if self.serie_anses else 0} meses")
        ...
        titulo_base = f"Evolución del haber Luz y Fuerza — {self.nombre_afiliado}" if self.nombre_afiliado else "Evolución del haber Luz y Fuerza"
        
        if self.comparar_con_anses and self.serie_anses:
            # Gráfico comparativo con dos líneas
            self.figura = grafico_comparativo_luz_fuerza_anses(
                fecha_inicio=self.periodo_desde,
                fecha_fin=self.periodo_hasta,
                monto_inicial=self.monto,
                serie_luz_fuerza=self.serie_montos,
                serie_anses=self.serie_anses,
                titulo=f"Comparación: Luz y Fuerza vs ANSES — {self.nombre_afiliado}" if self.nombre_afiliado else "Comparación: Luz y Fuerza vs ANSES")
        else:
            # Gráfico simple solo Luz y Fuerza
            self.figura = grafico_linea_haber_luz_fuerza(
                fecha_inicio=self.periodo_desde,
                fecha_fin=self.periodo_hasta,
                monto_inicial=self.monto,
                titulo=titulo_base)
    ...
    def generar_pdf(self):
        ...
        indices_mes = obtener_indices_luz_fuerza(self.periodo_desde, self.periodo_hasta)
        ...
        if getattr(self, "antiguedad", None):
            contexto["antiguedad"] = self.antiguedad
        ...
        if self.comparar_con_anses and self.monto_final_anses:
            ...
            if self.monto_final and self.monto_final_anses:
                if self.monto_final > self.monto_final_anses:
                    contexto["resultado_comparacion"] = "Movilidad Luz y Fuerza otorga mayor haber"
                    contexto["resultado_comparacion_clase"] = "lyf-mayor"
        ...
        html_rendered = render_template(
            "calculadora_luz_fuerza/resultado_calculadora_luz_fuerza.html",  # ruta de la plantilla
            **contexto)
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=
            f"resultado_haber_luz_fuerza_{self.numero_identificacion or 'afiliado'}.pdf",
