...
def crear_graficos(datos, etiquetas, titulo):
  ...
  fig = go.Figure(
      data=go.Bar(x=etiquetas,
                  y=valores,
                  marker_color=[
                      '#7671FA', '#00c4ff', '#E5EAF3', '#07244C', '#178DAD',
                      '#7E7F9C', '#9e73a3', '#3cd7c4', '#83007f', '#bb73b3'
                  ],
                  text=resultados,
                  textposition='auto',
...
class HerramientasDemanda:
  ...
  def __init__(self, datos_del_actor, expediente, cuil_expediente, beneficio,
               num_beneficio, comparacion_reparacion_historica,
               fecha_comparacion_repa, PBU_repa, PC_repa, PAP_repa, REPA_repa,
               OTROS_repa, PBU_reclamado, PC_reclamado, PAP_reclamado,
               REPA_reclamado, OTROS_reclamado, comparacion_repa_movilizado,
               comparacion_repa_movilizado_No, PBU_repa_movilizado,
               PC_repa_movilizado, PAP_repa_movilizado, REPA_repa_movilizado,
               OTROS_repa_movilizado, diccionario_sumas, tabla_pbu):
    ...
    self.tabla_pbu = tabla_pbu

  def obtener_datos_repa(self):
    diccionario = {
        'total_haber_con_repa': (self.PBU_repa + self.PC_repa + self.PAP_repa +
                                 self.REPA_repa + self.OTROS_repa),
        'total_haber_reclamado':
        (self.PBU_reclamado + self.PC_reclamado + self.PAP_reclamado +
         self.REPA_reclamado + self.OTROS_reclamado),
        'total_haber_con_repa_movilizado':
        (self.PBU_repa_movilizado + self.PC_repa_movilizado +
         self.PAP_repa_movilizado + self.REPA_repa_movilizado +
         self.OTROS_repa_movilizado),
    }

    # Agregar diferencias al diccionario después de calcular los totales
    diccionario['dif_reclamado_repa'] = (diccionario['total_haber_reclamado'] -
                                         diccionario['total_haber_con_repa'])
    diccionario['incidencia_reclamado_repa'] = str(
        round(
            diccionario['dif_reclamado_repa'] /
            diccionario['total_haber_con_repa'] * 100, 2)) + "%"
    diccionario['quita_confis_reclamado_repa'] = str(
        round(
            diccionario['dif_reclamado_repa'] /
            diccionario['total_haber_reclamado'] * 100, 2)) + "%"

    diccionario['dif_repa_movilizado_repa'] = (
        diccionario['total_haber_con_repa_movilizado'] -
        diccionario['total_haber_con_repa'])
    if diccionario['total_haber_con_repa_movilizado'] != 0:
      diccionario['incidencia_repa_movilizado_repa'] = str(
          round(
              diccionario['dif_repa_movilizado_repa'] /
              diccionario['total_haber_con_repa'] * 100, 2)) + "%"
      diccionario['quita_confis_repa_movilizado_repa'] = str(
          round(
              diccionario['dif_repa_movilizado_repa'] /
              diccionario['total_haber_con_repa_movilizado'] * 100, 2)) + "%"

    datos = [
        diccionario['total_haber_con_repa'],
        diccionario['total_haber_reclamado']
    ]
    ...
    grafico_repa = crear_graficos(
        datos, etiquetas, "Diferencia de Haber con Reparacion Historica")

    diccionario['total_haber_con_repa'] = formatear_dinero(
        diccionario['total_haber_con_repa'])
    diccionario['total_haber_reclamado'] = formatear_dinero(
        diccionario['total_haber_reclamado'])
    diccionario['total_haber_con_repa_movilizado'] = formatear_dinero(
        diccionario['total_haber_con_repa_movilizado'])

    diccionario['dif_reclamado_repa'] = formatear_dinero(
        diccionario['dif_reclamado_repa'])
    diccionario['dif_repa_movilizado_repa'] = formatear_dinero(
        diccionario['dif_repa_movilizado_repa'])
